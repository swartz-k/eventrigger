...
type SensorLister interface {
	...
	List(selector labels.Selector) (ret []*v1.Sensor, err error)
...
// List lists all Sensors in the indexer.
func (s *sensorLister) List(selector labels.Selector) (ret []*v1.Sensor, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Sensor))
	})
	return ret, err
}
...
type SensorNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.Sensor, error)
...
// List lists all Sensors in the indexer for a given namespace.
func (s sensorNamespaceLister) List(selector labels.Selector) (ret []*v1.Sensor, err error) {
...
// Get retrieves the Sensor from the indexer for a given namespace and name.
func (s sensorNamespaceLister) Get(name string) (*v1.Sensor, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("sensor"), name)
	}
	return obj.(*v1.Sensor), nil
}
